...
struct APIClient {
    static func getMoviesAPI(movieOption: String, page: Int, completion: @escaping (MovieJSON?) -> Void) {
        
        let url = URL(string: "https://api.themoviedb.org/3/movie/\(movieOption)?page=\(page)&language=es-ES&api_key=" + Constants.TMDBAPIKey)
